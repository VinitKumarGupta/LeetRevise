    ...
    try {
        ...
        if (status) {
            solvedProblems = solvedProblems.filter((item) => {
                if (status === "starred") return item.isStarred;
        ...
        if (sortBy) {
            items.sort((a, b) => {
                ...
                if (sortBy === "diff_asc" || sortBy === "diff_desc") {
                    ...
                    const weightA =
                        diffWeight[
                            a.problem.difficulty as "Easy" | "Medium" | "Hard"
                        ] || 0;
                    const weightB =
                        diffWeight[
                            b.problem.difficulty as "Easy" | "Medium" | "Hard"
                        ] || 0;
                    return sortBy === "diff_asc"
                        ? weightA - weightB
                        : weightB - weightA;
                }
                return 0;
    ...
    async (req: AuthenticatedRequest, res) => {
        try {
            ...
            if (!solvedProblem) {
                return res
                    .status(404)
                    .json({ message: "Problem tracking record not found" });
            }
        ...
        try {
            ...
            const updated = await db.solvedProblem.update({
                ...
                data: {
                    ...
                    status:
                        solvedProblem.status === "paused"
                            ? "active"
                            : solvedProblem.status,
                },
            });
...
router.patch(
    ...
    async (req: AuthenticatedRequest, res) => {
        try {
            const userId = req.user!.id;
            const id = req.params.id as string;
            const { starred } = toggleStarSchema.parse(req.body);
            ...
            return res.json({
                message: starred ? "Problem starred" : "Problem unstarred",
                solvedProblem: updated,
            });
        } catch (error: any) {
            if (error instanceof z.ZodError) {
...
router.delete(
    ...
    async (req: AuthenticatedRequest, res) => {
        try {
            ...
            return res.json({
                message: "Problem tracking removed successfully",
            });
        } catch (error) {
